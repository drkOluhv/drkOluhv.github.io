...
Similar to Example 10 the same result can be created with the lookup function with `=VLOOKUP(F1,A2:C5,2,FALSE)`. 
...
The syntax is the following: `=HLOOKUP(lookup_value, table_array, row_index_num, [range_lookup])` where the lookup value must be in the first row of the table.
...
The function follows the following syntax: `=XLOOKUP(lookup_value, lookup_array, return_array, [if_not_found], [match_mode], [search_mode])`
