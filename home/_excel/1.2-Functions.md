...
For this example to show the limitations of the functions that are mentioned the lookup value with now be the last name of Lachlan and return the middle name.For the Xlookup function the return value is on the left hand side of the lookup value which is ok for this function as it doesn't matter. Therefore the returned middle name is `James`
